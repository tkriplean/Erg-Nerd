...
Helper logic is split across:
  ...
  components/ranked_filters.py      — quality filters + season helpers
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  Chart rendered by RowingChart (components/rowing_chart.py), an hd.Plugin
  wrapping Chart.js with custom JS in rowing_chart_assets/rowing_chart.js.
...
from components.ranked_filters import (
...
from components.ranked_predictions import build_prediction_table_data
...
######################
# Hyperdiv overrides. See https://docs.hyperdiv.io/extending-hyperdiv/built-in-components
# Override hyperdiv's radio_group to add a size prop so that it can be passed
# to Shoelace as expected by Shoelace.
...
def ranked_tab() -> None:
    state = hd.state(
        ...
        chart_show_components=False,  # show per-anchor / component curves alongside the average
        chart_season_lines=(),
        chart_lines="PBs",  # "PBs" | "SBs" | "None" — which best-lines to draw
        show_chart_settings=False,
        # ---- timeline / simulation ---
        sim_playing=False,  # animation is running
        sim_week=_SIM_TODAY,  # day offset from sim_start; _SIM_TODAY = "today"
        sim_speed="1x",  # playback speed: one of _SPEED_OPTIONS
        sim_tick_id=0,  # monotonically-increasing; drives the tick scope key
        sim_last_pb_label="",  # text of most recently set PB during simulation
        sim_pb_set_at_day=-9999,  # sim_day_idx when the most recent PB was set
        sim_pb_stored_labels_json="[]",  # JSON-serialised overlay label dicts at detection time
        last_ds_change_id=0,  # last change_id seen from DateSlider; gates user-seek updates
        # ---- critical power model fit cache ---
        cp_fit_key="",  # fingerprint of the last-fitted PB data
        cp_fit_result=None,  # fitted param dict from fit_critical_power(), or None
    )
    ...
    if results_task.done and not results_task.error:
        all_ranked = [
            r for r in (results_task.result or []) if is_ranked_noninterval(r)
        ]
        all_ranked = apply_quality_filters(
            all_ranked,
            selected_dists=set(),  # quality filters are event-agnostic; event filter applied below
            selected_times=set(),
            excluded_seasons=set(),
        )

        # Capture after quality filters — the simulation uses this pre-filtered
        # set and simply gates by workout_date <= sim_date.
    ...
    if results_task.running:
        with hd.box(align="center", padding=4, gap=1):
            ...
            if progress.pages == 0:
                ...
                hd.text(
                    f"Page {progress.pages} fetched — "
                    f"{progress.total:,} workouts loaded so far…",
    ...
    # ----------------------------------------------------------- simulation timeline
    # Compute once per render; used by both the animation tick and the chart.
    ...
    if _included_seasons:
        ...
        # End of the latest included season (Apr 30 of the following year),
        # capped at today so we never show future dates.
        ...
        sim_end = date.today()
    _timeline_end_is_today = sim_end == date.today()
    ...
    # ----------------------------------------------------------- stable bounds
    # Derived from lifetime bests (one per event) for the current event+season
    # selection. Using bests rather than all raw workouts keeps the y range
    # sensible; the quality-filter threshold adds a small upward buffer so that
    # season-best performances visible during simulation don't clip the axis.
    show_watts = state.chart_metric == "Watts"
    # Derive line settings from the unified chart_lines radio
    ...
    # Reduce to one lifetime-best workout per event category before computing bounds.
    ...
    with shadowed_box(
        ...
        background_color="#252525" if hd.theme().is_dark else "#f9f9f9",
        border_radius="medium",
    ):
        # ---- header row ----
        _at_today = sim_day_idx >= total_days - 1
        ...
        with hd.hbox(gap=1, align="center", padding_bottom=0, justify="center"):
            ...
            hd.text(f"through ", font_size="medium")
            hd.text(f"{_date_label}", font_size="2x-large", font_weight="normal")

        # ---- RowingLevel scrape (normal mode only) ----
        # The scrape task always runs when profile is complete, regardless of which
        # chart predictor is selected.  The in-chart status indicators (spinner, error)
        # are only shown when the user has RL selected as the chart predictor.
        rl_predictions: dict = {}
        if _at_today:
            profile = load_profile()

            if not profile_complete(profile):
                # Only show the warning if the user has explicitly requested RL in chart
                if state.chart_predictor == "rowinglevel":
                    hd.alert(
                ...
                _lbest_dates: dict = (
                    {}
                )  # cat_key -> ISO date string of best performance
                ...
                with hd.scope(_scope_key):
                    ...
                    def _do_scrape(
                        gender, current_age, wkg, lbest, lbest_anchor, lbest_dates
                    ):
                ...
                if state.chart_predictor == "rowinglevel":
                    if rl_task.running:
                        with hd.hbox(gap=1, align="center", padding=1):
                            hd.spinner()
                            hd.text(
                                "Fetching RowingLevel predictions…",
                                font_color="neutral-500",
                                font_size="small",
                            )
                    elif rl_task.error:
                        hd.text(
                            f"RowingLevel error: {rl_task.error}",
                            font_color="danger",
                            font_size="small",
                        )
        ...
        # ---- chart render ----
        # Always build from sim_workouts_at — at today's date it's equivalent
        # to the normal filtered display set.

        # ---- animation tick (advances sim_week while playing) ----
        # Use sim_tick_id — not sim_week — as the scope key so that:
        #   (a) scrubbing the timeline backwards never revisits a "done" scope, and
        #   (b) the scope key is guaranteed to change on every advance even if
        #       sim_week happens to equal its previous value.
        # Always advance exactly 1 day per tick; speed controls the delay between ticks.
        # Minimum sleep is _BASE_TICK_SECS (gives HyperDiv time to stabilise the
        # re-render cascade); at higher multipliers the sleep shrinks proportionally.
        if state.sim_playing:
            with hd.scope(f"sim_tick_{state.sim_tick_id}"):
                ...
                if _tick.done:
                    ...
                    if _nd >= total_days:
                        state.sim_week = _SIM_TODAY  # return to "today"
                        state.sim_playing = False
                        # Do NOT reset sim_tick_id — it must stay above all
                        # previously-used scope values to avoid revisiting done tasks.
                    else:
                        state.sim_week = _nd
                        state.sim_tick_id += 1  # new id → new scope → fresh task
        ...
        # Lifetime bests from the current sim snapshot — drives the prediction table.
        # Computed here (inside the shadowed_box with block) so it is always available
        # in Python scope after the block exits; Python with-statements don't add scope.
        _lb, _lb_anchor = compute_lifetime_bests(sim_wkts)

        # Unfiltered lifetime bests (only gated on sim_date + excluded seasons, NOT on
        # event selection) — used for the "Your PB" column in the prediction table so
        # that PBs in events the user has hidden from the chart still appear.
        ...
        # ---- lookahead overlays (ghosts, arrows, threatened/new dots) ----
        # Only computed when we're in history (not at today) and lines are on.
        _sim_overlays = None
        _overlay_labels = []
        if not _at_today:
        ...
        # Always fit to lifetime PBs regardless of the best_filter setting.
        # Use workouts in the selected seasons/events up to the current sim_date.
        _cp_src = [
            w
            for w in all_ranked_raw
            if (w.get("distance") in selected_dists or w.get("time") in selected_times)
        ...
        _cp_fit_key = str(
            sorted(
                (round(p["duration_s"], 1), round(p["watts"], 1)) for p in _cp_pb_list
            )
        )
        ...
        # Width-capped and centered. Three-column layout keeps the slider
        # centered under the title regardless of button widths:
        #   [controls (fixed)] | [slider (grows)] | [spacer (= controls width)]
        with hd.box(align="center"):
            with hd.hbox(
                ...
                # ---- Left: Play + Speed (fixed width, mirrors right spacer) ----
                _CONTROLS_W = "9rem"
                with hd.hbox(gap=0.5, align="center", width=_CONTROLS_W):
                    ...
                    if hd.button(
                        ...
                        if state.sim_playing:
                            ...
                            if _at_today:
                                state.sim_week = 0  # restart from beginning
                            state.sim_tick_id += 1
                            state.sim_playing = True

                    # Speed — click cycles through options
                    _sp_idx = (
                ...
                # ---- Center: Scrubber (grows) ----
                # change_id gates updates so Python-side sim_week changes
                # (buttons, animation ticks) are never overwritten by a stale value.
                with hd.box(grow=1):
                    ...
                    ds = DateSlider(
                        ...
                        target_value=sim_day_idx,  # Python-owned: always drives thumb
                        step=1,
                        start_date=sim_start.isoformat(),
                        annotations=_sb_annotations,
                        # value and change_id are JS→Python only; never passed here.
                    )
                    if ds.change_id != state.last_ds_change_id:
                        state.last_ds_change_id = ds.change_id
                        state.sim_week = int(ds.value)
                        state.sim_playing = False  # seeking pauses playback

                # ---- Right: spacer mirrors left column to keep slider centered ----
                with hd.box(width=_CONTROLS_W):
                    pass

        # ---- chart ----
        if chart_cfg:
            RowingChart(config=chart_cfg, show_watts=show_watts, height="75vh")
        else:
            hd.text("No chart data available.", font_color="neutral-500")

        # ---- chart settings (below graph, centered) ----
        with hd.hbox(
            ...
            # MP4 export is parked in services/mp4_export.py for future re-enablement.

        # "Show components" row — only when predictor supports it
        if state.chart_predictor in ("pauls_law", "rowinglevel", "critical_power"):
            with hd.hbox(gap=1, align="center", justify="center", padding_top=0.25):
    ...
    # ----------------------------------------------------------- prediction table
    # Show whenever any row can be produced (i.e. there are ranked PBs in scope).
    # All four predictor columns are always present; cells show "—" when unavailable.
    # rl_predictions is populated only when the RL scrape has completed.
    ...
    # Only render if at least one row has any data at all
    if any(
        ...
        with hd.box(padding=(2, 0, 0, 0)):
            ...
            # ── custom table (hd.data_table can't host tooltip icons in headers) ──
            _PRED_COLS = [
                # (key_prefix, header_label, tooltip_text)
                (
                    "pb",
                    "Your PB",
                    "Your personal best for each event.",
                ),
                (
                    "cp",
                    "Critical Power",
                    "Two-component power-duration model (veloclinic). "
                    "Requires ≥ 5 PBs spanning a 10:1 duration ratio. "
                    "Method from rowsandall.com.",
                ),
                (
                    "loglog",
                    "Log-Log Watts Fit",
                    "Fits a power law (log watts vs log distance) across all "
                    "scoped PBs. Similar to the Free Spirits Pace Predictor "
                    "(freespiritsrowing.com) but uses all PBs, not just two.",
                ),
                (
                    "pl",
                    "Avg. Paul's Law",
                    "Predicts +5 s/500m for each doubling of distance, "
                ...
                (
                    ...
                    "(gender, age, bodyweight). Averaged across all anchor PBs. "
                    "Requires profile completion. Distance events only.",
                ),
                (
                    "avg",
                    "Average",
                    "Mean of all available predictions for this event.",
                ),
            ]

            _HEADER_BG = "neutral-100"
            # flex: 1 1 0 (grow=True, width=0) forces every column to the same width
            # regardless of content — the flexbox distributes space from a zero basis.
            _COL_PROPS = dict(
                grow=True, width=0, padding=0.5, border_right="1px solid neutral-200"
            )

            # One outer box holds the whole table; overflow:hidden clips inner
            # border_right on the last column so it doesn't double with the outer border.
            with hd.box(
                border="1px solid neutral-200",
                border_radius="medium",
            ):
                ...
                # ── data rows ─────────────────────────────────────────────────
                for _ri, _row in enumerate(_pred_rows):
                    with hd.scope(_row["label"]):
                        _row_bg = "neutral-50" if _ri % 2 == 0 else "neutral-0"
                        _pb_raw = _row.get("pb_raw")  # float or None

                        # Determine enabled state for this event row
                        ...
                        with hd.hbox(
                            ...
                            for col_key, col_label, _tip in _PRED_COLS:
                                with hd.scope(col_key):
                                    ...
                                    # Delta vs PB — only for prediction cols,
                                    # not the PB column itself
                                    has_delta = (
                                        col_key != "pb"
                                    ...
                                    # Dim the PB column when the event is disabled
                ...
                # ── accuracy row ──────────────────────────────────────────────
                # RMSE + R² in sec/500m, filtered to enabled events only.
                _acc_vals: dict = {}
                for _ck in ["avg", "cp", "loglog", "pl", "rl"]:
                    ...
                    if _pairs:
                        ...
                        _ss_tot = sum(
                            (a - _mean_actual) ** 2 for a in _actuals_v
                        )
                        _acc_vals[_ck] = {
                            "rmse": (_ss_res / len(_pairs)) ** 0.5,
                            "r2": 1.0 - _ss_res / _ss_tot
                            if _ss_tot > 0
                            else None,
                            "n": len(_pairs),
                        }
