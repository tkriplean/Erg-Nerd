...
When ``x_min``/``x_max`` are supplied the bars and user marker are positioned
in that *global* domain (rather than each histogram's local min/max). This
lets several histograms rendered side-by-side share a common x-axis for
visual comparison across events.
"""
...
def distribution_svg(
    ...
    * ``user_watts`` — drawn as a dashed vertical line. If outside
      the active x-domain the line clamps to the nearest edge.
    * ``x_min``/``x_max`` — optional global x-axis domain. When set, bars are
      positioned within that range; otherwise the local [min_watts, max_watts]
      is used.
    * Empty histograms render as a single placeholder bar.
    ...
    dom_lo = x_min if x_min is not None else min_watts
    dom_hi = x_max if x_max is not None else max_watts
    if dom_hi <= dom_lo:
        dom_hi = dom_lo + 1.0

    def _map_x(w: float) -> float:
        return (w - dom_lo) / (dom_hi - dom_lo) * width
    ...
    if nbins > 0 and max_count > 0 and max_watts > min_watts:
        bin_width_w = (max_watts - min_watts) / nbins
        for i, c in enumerate(bin_counts):
            ...
            if h < 1:
                h = 1
            bin_lo = min_watts + i * bin_width_w
            bin_hi = bin_lo + bin_width_w
            x0 = _map_x(bin_lo)
            x1 = _map_x(bin_hi)
            bar_w = max(0.5, x1 - x0)
            y = height - h
    ...
    # User marker.
    uw = max(dom_lo, min(dom_hi, user_watts))
    x = _map_x(uw)
    marker = (
