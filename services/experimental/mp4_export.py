...
from performance_page.py and wiring mp4_task = hd.task() back in.

To re-enable:
  1. In performance_page.py, add `mp4_task = hd.task()` inside performance_page().
...
# ---------------------------------------------------------------------------
# HyperDiv UI wiring (parked — uncomment and call from performance_page to re-enable)
# ---------------------------------------------------------------------------
...
def render_mp4_export_button(state, mp4_task, sim_config: dict) -> None:
    ...
    Call this from the chart-settings row in performance_page() after adding
