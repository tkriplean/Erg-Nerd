...
_enrich_workouts(all_workouts, thresholds, max_hr)
    ...
    ↓
_filter_disjunctive(… active_bins, power_bin_passes, "_bin_meters")
...
_zone_filter_legends(state, max_hr)    — Power + HR chip legends (OR within each)
...
Power zones (`services/volume_bins.py`) — 7 bins (index 0 = Rest). Boundaries are watts, derived from reference-watts at 1k, 2k, 5k, 60min, and marathon:

| Index | Name | Description |
|---|---|---|
| 0 | Rest | Interval rest distance only — excluded from intensity |
| 1 | Fast | Above midpoint(1k, 2k) watts |
...
| 5 | Fast Aerobic | Down to ~3 s/500m slower than marathon pace (translated to watts) |
| 6 | Slow Aerobic | Below the Fast-Aerobic boundary |
...
- Power weights: `[0, 100, 80, 60, 40, 20, 0]` (Fast → Slow Aerobic)
...
```python
{
    ...
    "expected_score":   65,    # target power-intensity score (anchored to today's fitness)
...
Selecting multiple chips in the Power legend shows workouts whose power distribution passes the zone threshold for **any** selected zone (union). Same for HR. The three filter groups combine conjunctively with each other:

```
visible = grid_cells_filter(w) AND power_chips_filter(w) AND hr_chips_filter(w)
```
...
Power (`power_bin_passes` — `services/volume_bins.py`):
...
Each chip carries an `hd.tooltip` whose `content_slot` box contains: bold zone heading, one-line definition (`POWER_ZONE_DEFINITION_TEXT` / `HR_ZONE_DEFINITION_TEXT`), and one-line filter rule (`POWER_ZONE_FILTER_TEXT` / `HR_ZONE_FILTER_TEXT`).
...
| `active_bins` | `tuple[int]` | Selected power bins 1–6. OR filter. |
...
```
all_intervals
  → _filter_disjunctive (power chips, _bin_meters)
...
**To change power filter thresholds**: edit `power_bin_passes()` in `services/volume_bins.py`; update `POWER_ZONE_FILTER_TEXT` so chip tooltips stay honest.
